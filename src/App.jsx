...
export default function App() {
  ...
  const canSync = Math.floor((Date.now() - new Date(day.date + "T12:00:00").getTime()) / 86400000) <= 30;
            ...
            <div className="card" style={{marginBottom:16,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <div style={{flex:1,minWidth:0}}>
                ...
                {day.oura ? (
                  ...
                  <div style={{fontSize:10,color:"#8899aa"}}>{GYM_MAP[day.gym]>0?`${GYM_MAP[day.gym]} kcal (manual)`:"Sin datos de actividad"}</div>
                )}
                {ouraError && <div style={{fontSize:9,color:"#f87171",marginTop:3}}>{ouraError}</div>}
              </div>
              {canSync && <button onClick={()=>fetchOura(day.date)} disabled={ouraLoading} style={{background:"#0e1a26",border:"1px solid #38bdf8",color:"#38bdf8",borderRadius:6,padding:"6px 12px",cursor:"pointer",fontFamily:"'JetBrains Mono',monospace",fontSize:10,flexShrink:0,marginLeft:10}}>{ouraLoading?"↻":day.oura?"↻ Sync":"Sync"}</button>}
            </div>
...
export default function App() {
  ...
  const avgCals = Math.round(totalCals / trend.length);
  const avgBalance = Math.round(totalBalance / trend.length);
  const [ady, adm, add] = day.date.split("-").map(Number);
  const canSync = Math.floor((new Date() - new Date(ady, adm - 1, add)) / 86400000) <= 30;
      ...
      <div style={{ maxWidth: 500, margin: "0 auto", padding: "20px 20px" }}>
        ...
        {tab === "log" && (
          <div className="fi">
            {/* Oura Sync card */}
            {(() => {
              const [y, mo, dy] = day.date.split("-").map(Number);
              const dayDate = new Date(y, mo - 1, dy);
              const diffDays = Math.floor((new Date() - dayDate) / 86400000);
              const canSync = diffDays <= 30;
              return (
              <div className="card" style={{ marginBottom: 16, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 9, color: "#8899aa", letterSpacing: ".1em", textTransform: "uppercase", marginBottom: 4 }}>Oura Ring</div>
